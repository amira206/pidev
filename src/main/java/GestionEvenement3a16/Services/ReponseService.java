...
public class ReponseService {
    ...
    public void updateEtatReclamation(int reclamationId, String newEtat) {
        String req = "UPDATE reclamation SET etat = '" + newEtat + "' WHERE id = " + reclamationId;
        try {
            Statement st = cnx.createStatement();
            st.executeUpdate(req);
            System.out.println("Reclamation " + reclamationId + " updated to " + newEtat);
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    }

...
import GestionEvenement3a16.Cours.Controller.CategoryListController;
...
public class SampleController implements UserCardRefreshListener {
    ...
    private void exportToPdf() {
        ...
        try {
            ...
            float[] columnWidths = {1f, 2f, 2f, 3f, 2f, 3f, 2f, 2f};
            table.setWidths(columnWidths);

            // Change opacity of color
            BaseColor headerColor = new BaseColor(0, 0, 0); // RGB
            Font headerFont = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD, headerColor);
            PdfPCell cell;

            String[] headers = {"ID", "First Name", "Last Name", "Email", "Phone", "Address", "CreatedDT", "Avatar"};
            for (String header : headers) {
                cell = new PdfPCell(new Phrase(header, headerFont));
                BaseColor borderColor = new BaseColor(204, 255, 255); // RGB
                cell.setBackgroundColor(borderColor);            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                table.addCell(cell);
            }

            // Different color for ID
            Font idFont = new Font(Font.FontFamily.HELVETICA, 12, Font.NORMAL, BaseColor.RED);

            for (User user : users) {
                cell = new PdfPCell(new Phrase(String.valueOf(user.getId()), idFont));
                table.addCell(cell);
                ...
                // Add avatar image to the cell
                URL avatarUrlResource = getClass().getResource(user.getAvatar());
                if (avatarUrlResource != null) {
                    com.itextpdf.text.Image avatar = com.itextpdf.text.Image.getInstance(avatarUrlResource.toExternalForm());
                    avatar.scaleToFit(50f, 50f);
                    cell = new PdfPCell();
                    cell.addElement(avatar);
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    table.addCell(cell);
                } else {
                    table.addCell("No avatar");
                }
            }
            ...
            rect.setBorderWidth(2); // a width of 2
